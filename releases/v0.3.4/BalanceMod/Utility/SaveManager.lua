...
function BalanceMod.RestoreModData()
    if shouldRestoreOnUse then
        ...
        local newData = BalanceMod.DeepCopy(dataCacheBackup)
        dataCache.run = BalanceMod.PatchSaveTable(newData, BalanceMod.DefaultRunSave())
        dataCache.hourglassBackup = BalanceMod.PatchSaveTable(newData, BalanceMod.DefaultRunSave())
    end
end
...
local function ResetRunSave()
    dataCache.run = BalanceMod.DefaultRunSave()
    dataCache.hourglassBackup = BalanceMod.DefaultRunSave()
    dataCacheBackup = BalanceMod.DefaultRunSave()
...
BalanceMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
    if not skipNextRoomClear then
        dataCacheBackup.persistent = BalanceMod.DeepCopy(dataCache.run.persistent)
        dataCacheBackup.room = BalanceMod.DeepCopy(dataCache.run.room)
        dataCache.run.room = BalanceMod.DeepCopy(BalanceMod.DefaultRunSave().room)
        BalanceMod.SaveModData()
        shouldRestoreOnUse = true
    end
...
BalanceMod:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, function()
    if not skipNextLevelClear then
        dataCacheBackup.persistent = BalanceMod.DeepCopy(dataCache.run.persistent)
        dataCacheBackup.level = BalanceMod.DeepCopy(dataCache.run.level)
        dataCache.run.level = BalanceMod.DeepCopy(BalanceMod.DefaultRunSave().level)
        BalanceMod.SaveModData()
        shouldRestoreOnUse = true
    end
