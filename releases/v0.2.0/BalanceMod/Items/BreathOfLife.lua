...
local ShaderHandler = require("BalanceMod.Utility.ShaderHandler")
...
function BreathOfLife:PostRender()
    local lowestMeter
...
return function (BalanceMod)
    ...
    if EID then
        EID:addCollectible(BreathOfLife.Item, "Isaac becomes invincible while his charge is completely depleted#Deplete charge by holding use#{{Warning}} Holding for too long after becoming invincible will deal damage every second#Charge regenerates when not held#{{Collectible160}} Touching enemies while invincible summons a beam of light to damage them") 
    end

    ShaderHandler.AddShaderCondition("Vignette", 1.0, function ()
        return VignetteReturn
    end)    
