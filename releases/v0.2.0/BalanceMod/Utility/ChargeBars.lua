...
---@param sprite Sprite
function ChargeBar:MakeCustomChargeBar(player, sprite, charge, maxCharge, incrementValue, ignoreOffset) -- returns index of charge bar in the player's ChargeBarData ~ increment value is how much the charge goes up per frame
...
function ChargeBar:ResetCharge(player, barId)
    local playerIndex = GetPlayerIndex(player)
    local playerData = ChargeBarData[playerIndex]
...
function ChargeBar:GetChargeBar(player, barId)
    local playerIndex = GetPlayerIndex(player)
    local playerData = ChargeBarData[playerIndex]
...
function ChargeBar:UpdateCustomChargeBar(player, barId) -- draws and increments charge bar
    ...
    local overlapOffset = bar.ignoreOffset and Vector.Zero or (ChargeBarOverlapOffset * (bar.Index - 1)) -- move the bar depending on how where it is in the array
