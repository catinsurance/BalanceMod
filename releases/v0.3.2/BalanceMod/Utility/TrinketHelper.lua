...
function Exhaustion:SpawnTrinket(trinketId, position, velocity)
    whitelist[tostring(trinketId)] = true
...
function Exhaustion:OnTrinketInit(pickup)
    ...
    if whitelist[tostring(pickup.SubType)] then
        trinketShouldBeRemoved = false
        whitelist[tostring(pickup.SubType)] = nil
        pickup:GetData().SpawnedByBalanceMod = true
    end
