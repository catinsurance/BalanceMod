...
local function morphTrinket(player)
    for _, trinket in ipairs(removedTrinkets) do
        if player:HasTrinket(trinket, true) then
...
---@param player EntityPlayer
function Exhaustion:PeffectUpdate(player)
    morphTrinket(player)
end
...
BalanceMod:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, Exhaustion.PeffectUpdate)
